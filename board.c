...
int is_valid_move(int r1, int c1, int r2, int c2) { // checks if the move is valid aka in bounds already taken or 2 dots way too far appart 
    if (r1 < 0 || r1 >= ROWS ||
        r2 < 0 || r2 >= ROWS ||
        c1 < 0 || c1 >= COLS ||
        c2 < 0 || c2 >= COLS) {
    ...
    if (r1 == r2) {
        int c = (c1 < c2) ? c1 : c2;
        if (h_lines[r1][c]) return 0;
    } else {
        int r = (r1 < r2) ? r1 : r2;
        if (v_lines[r][c1]) return 0;
    }
...
int get_all_valid_moves(Move moves[], int maxMoves) {
    ...
    for (int r = 0; r < ROWS; r++) {
        for (int c = 0; c < COLS; c++) {
            if (c + 1 < COLS && is_valid_move(r, c, r, c + 1)) {// right boxes
                if (count < maxMoves)
                    moves[count++] = (Move){r, c, r, c + 1};
            }
            if (r + 1 < ROWS && is_valid_move(r, c, r + 1, c)) {// down
                if (count < maxMoves)
...
int would_complete_box(int r1, int c1, int r2, int c2) { // new way for bot to simulate moves to fix scoring issue
    int completed = 0;
